...
public class TestExceptionCases {
    ...
    public void TestRemoveProductInternallyThrowsExceptionWhenProductNotFoundLab() {
        ...
        var ex = Assert.Throws<Exception>(() => lab.RemoveProductInternally(product));
        Assert.That(ex.Message, Is.EqualTo("Product not found."));
    }
    ...
    public void TestAddCompositionAssociationThrowsExceptionWhenEquipmentAlreadyAttached() {
        ...
        var ex = Assert.Throws<Exception>(() => lab.AddCompositionAssociation(equipment));
        Assert.That(ex.Message, Is.EqualTo("Equipment already attached to the lab."));
    }
    ...
    public void TestRemoveCompositionAssociationThrowsExceptionWhenEquipmentNotAttached() {
        ...
        var ex = Assert.Throws<Exception>(() => lab.RemoveCompositionAssociation(equipment));
        Assert.That(ex.Message, Is.EqualTo("Equipment has not been attached to the lab yet."));
    }
    
    [Test]
    public void TestRemoveIngredientInternallySuccessfullyRemovesIngredientInstruction()
    {
        var instruction = new Instruction("Stir the mixture");
        var ingredient = new Ingredient("Salt", 5, "h20", StateAttribute.Liquid);
        
        instruction.AddIngredient(ingredient);
        
        instruction.RemoveIngredientInternally(ingredient);
        
        Assert.IsFalse(instruction.AssociatedIngredients.Contains(ingredient), "Ingredient should be removed from the associated ingredients.");
    }
